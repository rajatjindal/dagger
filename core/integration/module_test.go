...
func (GoSuite) TestModuleNamingCompatDependencyTestcase1(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)
...
// foo: dev; bar: v0.12.5 (dependency has old engine version)
func (GoSuite) TestModuleNamingCompatDependencyTestcase2(ctx context.Context, t *testctx.T) {
	c := connect(ctx, t)
...
current status for testcase - 3

foo: v0.12.5
bar: dev

in foo/internal/dagger/dagger.gen.go - skipTParse
in /schema.json -> skipTParse

but getting error:

invoke: input: bar.withSecondFunction resolve: failed to set call inputs: failed to find arg "skipTParse"
			, normalized "skipTParse"
			, . all map[string]*core.UserModFunctionArg{"skipTparse":(*core.UserModFunctionArg)(0x4000eaecd8)}
			. strcase -> *strcase.LegacyCaser

step 2:

changed setCallInputs function in modfunc.go to use normalized name, like we were doing earlier. lets see what happens now:
this fixed it.

BUT

lets try Testcase1 and testcase 4 again to be sure
testcase 1 -> works
testcase 2 -> works too?
testcase 3 -> works
testcase 4 -> works
...
func (GoSuite) TestModuleNamingCompatDependencyTestcase3(ctx context.Context, t *testctx.T) {
	...
	// TODO(rajatjindal): because we are installing module as per its engine version
	// then, the codegen should generate code as per the dependency's engine version as well
	// this might be our issue here.
	// so need to debug codegen now
	// out, err := devModGen.
	// 	WithExec([]string{"grep", "-i", "skip", "internal/dagger/dagger.gen.go"}).
	// 	Stdout(ctx)
